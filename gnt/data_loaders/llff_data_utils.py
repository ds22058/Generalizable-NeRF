...
def get_poses(basedir):


    with open(os.path.join(basedir + '/poses_bounds.json'), 'r', encoding='utf-8') as fp:
        data = json.load(fp)
        poses = []
        num = len(data['rvec_np'])
        near = data['near']
        far = data['far']
        focal = data['focal']
        h = data['h']
        w = data['w']
        hwf = [h, w, focal]
        near_far = np.array([near, far] * num).reshape(num, 2)
        trans = np.matrix([[0, 1, 0], [1, 0, 0], [0, 0, -1]])
        for i in range(num):
            R = np.array(data['rvec_np'][i]).reshape(-1,3)
            T = np.array(data['tvec_np'][i]).reshape(3,-1)/100
            p = np.matrix(np.append(R, T, axis=1))
            p = np.array(np.dot(trans, p))
            p = np.append(p, np.array([0,0,0,1]).reshape(1,4),axis=0)
            p_ = np.array(np.matrix(p).I)
            p_ = np.append(p_[:3,:], np.array(hwf).reshape(3, 1),axis=1)
            poses.append(p_)

        poses = np.append(np.array(poses).reshape(num,15), near_far, axis=1)
        np.save(os.path.join(basedir, 'poses_bounds.npy'), poses)

def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, npy = False):
...
def load_llff_data(
    ...
    if spherify:
        ...
        #mark here for correct
        close_depth, inf_depth = bds.min() * 0.9, bds.max() * 5.0
        # close_depth, inf_depth = bds.min() * 5.0, bds.max() * 1.0
        dt = 0.75
