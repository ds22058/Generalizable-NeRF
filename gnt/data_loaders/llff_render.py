...
class LLFFRenderDataset(Dataset):
    def __init__(self, args, scenes="fern", **kwargs):
        ...
        for i, scene in enumerate(scenes):
            ...
            #mark here for change factor from 4 to 2
            ###mark here for correct
            near_depth = np.min(bds)
            #near_depth = 0.5 * (np.min(bds) + np.max(bds))
    ...
    def __getitem__(self, idx):
        ...
        nearest_pose_ids = get_nearest_pose_ids(
            ...
            tar_id=id_render,
            # mark here for change dist to vector
        ...
        depth_range = torch.tensor([depth_range[0] * 0.8, depth_range[1] * 1.8])
