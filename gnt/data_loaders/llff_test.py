...
class LLFFTestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.args.few_shot_input:
            train_intrinsics = list(train_intrinsics)
            train_poses = list(train_poses)
            train_rgb_files.pop(idx)
            train_intrinsics.pop(idx)
            train_poses.pop(idx)
            train_intrinsics = np.array(train_intrinsics)
