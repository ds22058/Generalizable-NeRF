...
def render_rays(
    ...
    #mark here for adding frequency regulartion
    mask_rate = 60*(global_step/total_step)
    rgb = model.net_coarse(rgb_feat, ray_diff, mask, pts, ray_d, mask_rate)
    if ret_alpha:
    ...
    if N_importance > 0:
        ...
        if single_net:
            rgb = model.net_coarse(rgb_feat_sampled, ray_diff, mask, pts, ray_d, mask_rate)
        else:
            rgb = model.net_fine(rgb_feat_sampled, ray_diff, mask, pts, ray_d, mask_rate)
        rgb, weights = rgb[:, 0:3], rgb[:, 3:]
        depth_map = torch.sum(weights * z_vals, dim=-1)
