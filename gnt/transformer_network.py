...
# **kwargs dictionary pack or split
# *arg tuple pack or splilt
...
#     def __init__(self, dim, norm_act=nn.BatchNorm3d):
#         super(MinCostRegNet, self).__init__()
#         self.conv0 = ConvBnReLU3D(dim, dim*2, norm_act=norm_act)
#
#         self.conv1 = ConvBnReLU3D(dim*2, dim*2, stride=2, norm_act=norm_act)
#         self.conv2 = ConvBnReLU3D(dim*2, dim*2, norm_act=norm_act)
#
#         self.conv3 = ConvBnReLU3D(dim*2, dim*4, stride=2, norm_act=norm_act)
#         self.conv4 = ConvBnReLU3D(dim*4, dim*4, norm_act=norm_act)
#
#         self.conv9 = nn.Sequential(
#             nn.ConvTranspose3d(dim*4, dim*2, 3, padding=1, output_padding=1,
#                                stride=2, bias=False),
#             norm_act(dim*2))
#
#         self.conv11 = nn.Sequential(
#             nn.ConvTranspose3d(dim*2, dim, 3, padding=1, output_padding=1,
#                                stride=2, bias=False),
#             norm_act(dim))
#
#         # self.depth_conv = nn.Sequential(nn.Conv3d(16, 1, 3, padding=1, bias=False))
#         self.feat_conv = nn.Sequential(nn.Conv3d(dim, dim, 3, padding=1, bias=False))
#
#     def forward(self, x):
#         conv0 = self.conv0(x)
...
class GNT(nn.Module):
    def __init__(self, args, in_feat_ch=32, posenc_dim=3, viewenc_dim=3, ret_alpha=False):
        ...
        # self.cost_reg_nets = nn.ModuleList([])
        ...
        for i in range(args.trans_depth):
            ...
            #cost_reg_net
            # cost_reg_nets = MinCostRegNet(args.netwidth)
            # self.cost_reg_nets.append(cost_reg_nets)
            # ray transformer
    ...
    def forward(self, rgb_feat, ray_diff, mask, pts, ray_d, mask_rate):
        # compute positional embeddings
        viewdirs = ray_d
        viewdirs = viewdirs / torch.norm(viewdirs, dim=-1, keepdim=True)
        viewdirs = torch.reshape(viewdirs, [-1, 3]).float()
        viewdirs = self.view_enc(viewdirs)#after encode 3->63
        pts_ = torch.reshape(pts, [-1, pts.shape[-1]]).float()
        pts_ = self.pos_enc(pts_)#3->63
        pts[:, int(60*mask_rate+3):int(60*mask_rate+6)] = (mask_rate - int(mask_rate))*pts[:, int(60*mask_rate+3):int(60*mask_rate+6)]
        pts[:, int(mask_rate+6):] = 0
        ...
        rgb_feat = self.rgbfeat_fc(rgb_feat)#[N__rays,N_samples,N-views,channels=35]->[N__rays,N_samples,N-views,channels=64]
        #mark here for add mvs
        # volume_sum = 0
        # volume_sq_sum = 0
        # for view in range(rgb_feat.shape[2]):
        #     volume_sum = volume_sum + rgb_feat[:, :, view, :]
        #     volume_sq_sum = volume_sq_sum + rgb_feat[:, :, view, :]**2
        # feature_volume = volume_sq_sum.div_(rgb_feat.shape[2]).sub_(volume_sum.div_(rgb_feat.shape[2]).pow_(2))
        # q_init -> maxpool
        q = rgb_feat.max(dim=2)[0]
        # q = feature_volume
        # q = self._nets(q)
        # transformer modules
        #enumerate bianli jia suoyin
        #zip turn into tuple
